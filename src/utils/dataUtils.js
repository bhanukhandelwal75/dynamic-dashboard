...
export function detectCols(headers) {
  ...
  return {
    ...
    custType:    find('customer_type', 'custtype', 'clienttype'),
    custName:    find('customer_name', 'custname', 'clientname'),
    custId:      find('customer_id', 'custid', 'clientid', 'kyb'),
    createdDate: find('created_date', 'createdate', 'creationdate', 'createdat', 'opendate'),
    lastAction:  find('last_action_date', 'lastactiondate', 'lastaction', 'updateddate', 'closeddate'),
    comments:    find('comments', 'comment', 'remarks', 'notes','Comments'),
    caseId:      find('casenumber', 'case_id', 'ticketid'),
    month:       find('month1', 'month', 'reportingmonth', 'period'),
    actionType:  find('action_type', 'actiontype'),
    ageing:      find('ageing', 'aging'),
    l3User:      find('l3_user', 'l3user'),
...
export function enrichRow(r, CM) {
  ...
  const commentStr = CM.comments ? String(r[CM.comments] || '') : '';
  // This regex looks for "MID -" followed by any alphanumeric characters
  const cleanComments = commentStr.replace(/<\/?[^>]+(>|$)/g, "");

  // 2. Look for MID followed by a dash or space, then the value
  // This version is more flexible with spaces and special characters
  const midMatch = cleanComments.match(/MID\s*[-:]\s*([A-Za-z0-9]+?)(?=Entity|$)/i);
  
  const extractedMID = midMatch ? midMatch[1] : '—';
  ...
  const statusRaw = (r[CM.status] || '').toLowerCase().trim();

  const isSTR = statusRaw === 'raise str';
  const isClosed = statusRaw === 'close' || statusRaw === 'close - previous sar' || statusRaw.includes('complet');
